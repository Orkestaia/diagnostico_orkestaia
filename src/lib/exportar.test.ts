...
import { exportJson, exportMarkdown, nombreArchivo, valorLegible, type FilaExport } from "./exportar";
...
const privado = {
  campos: { "c.coste_perfil": { operativo: 18, tactico: 25, directivo: 40, origen: "cliente" }, "e.inversion": "1.000-3.000 €" },
  procesos: { "previo-dia-expedientes-mes": { pctAutomatizable: 0.5, nota: "Usan Drive compartido" } },
};
...
const fila: FilaExport = {
  ...
  respuestas_previo: { "dia.expedientes_mes": "5-15", "herramientas.lista": ["Software de mi sector"], "herramientas.lista::Software de mi sector": "A3" },
  respuestas_visita: {
    inicio_at: "2026-09-22T08:00:00Z",
    campos: { "a.historia": "Despacho familiar desde 1990", "a.equipo": [{ rol: "Abogados", numero: 3 }], "e.prioridades": ["previo-dia-expedientes-mes"] },
  ...
  calculo: {
    ...calcularVisita(procesos, privado.procesos, { sector: "servicios_profesionales", personas: 3, costes: costesConCorreccion({ operativo: 18, tactico: 25, directivo: 40 }) }),
...
describe("export para JARVIS", () => {
  ...
  it("sugiere el nombre con la fecha de la visita", () => {
    ...
    expect(nombreArchivo({ visita_cerrada_at: null, fecha_reunion: null }, "2026-09-23", "json")).toBe("2026-09-23_diagnostico-app_raw.json");
  });
  ...
  it("sin visita cerrada lo dice en vez de inventar cifras", () => {
    expect(exportMarkdown({ ...fila, calculo: null }, "2026-09-23")).toContain("todavía no hay cálculo");
  });
